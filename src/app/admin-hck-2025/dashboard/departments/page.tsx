...
interface Dept {
  ...
  icon_name?: string; image_url?: string; color?: string;
  is_active: boolean; order_index: number;
  hod_name?: string; hod_title?: string; hod_bio?: string;
}
...
function Modal({ dept, onClose, onSave }: {
  dept: Partial<Dept>; onClose: () => void; onSave: (d: Partial<Dept>) => Promise<void>
}) {
  ...
  const uploadImg = async (file: File) => {
    setUploading(true);
    const fd = new FormData(); fd.append('file', file); fd.append('folder', 'departments');
    try {
        ...
        <div className="p-6 space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div><label className="block text-xs font-semibold text-[#0F2340] mb-1">Slug *</label>
              <input value={form.slug ?? ''} onChange={e => set('slug', e.target.value)} className={inputCls} placeholder="emergency" /></div>
          </div>
          ...
          {/* Image upload */}
          <div>
            <label className="block text-xs font-semibold text-[#0F2340] mb-1">Department Image</label>
            {form.image_url && <img src={form.image_url} alt="" className="w-full h-32 object-cover rounded-lg mb-2" />}
            <div className="flex gap-2">
              <input value={form.image_url ?? ''} onChange={e => set('image_url', e.target.value)}
                className={inputCls + ' flex-1'} placeholder="Image URL or upload below" />
          ...
          <div className="bg-[#F8FAFF] rounded-xl p-4 border border-[#D1DCF5]">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div><label className="block text-xs text-[#8896B3] mb-1">Name</label>
                <input value={form.hod_name ?? ''} onChange={e => set('hod_name', e.target.value)} className={inputCls} /></div>
              <div><label className="block text-xs text-[#8896B3] mb-1">Title</label>
                <input value={form.hod_title ?? ''} onChange={e => set('hod_title', e.target.value)} className={inputCls} /></div>
            </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-[#2D2D2D]">Active / Visible on site</span>
          </label>
        </div>
...
export default function DepartmentsManager() {
  ...
  const save = async (form: Partial<Dept>) => {
    try {
      const r = await fetch(form.id ? `/api/admin/departments/${form.id}` : '/api/admin/departments', {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-[#D1DCF5] overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-[#8896B3]"><RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2 opacity-40" /><p className="text-sm">Loading from Supabase…</p></div>
        ) : (
                  ...
                  <tr key={d.id} className="hover:bg-[#FAFBFF] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setModal(d)} className="p-1.5 rounded-lg text-[#0F2340] hover:bg-[#EBF0FB]"><Edit3 className="w-3.5 h-3.5" /></button>
                        <button onClick={() => toggle(d.id)} className={`p-1.5 rounded-lg ${d.is_active ? 'text-green-600 hover:bg-green-50' : 'text-[#8896B3] hover:bg-[#F0F4FF]'}`}>
                          {d.is_active ? <ToggleRight className="w-3.5 h-3.5" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                        </button>
                        <button onClick={() => remove(d.id)} className="p-1.5 rounded-lg text-[#C8102E] hover:bg-red-50"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
