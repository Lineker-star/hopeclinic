...
import { Plus, Edit3, Trash2, X, Save, RefreshCw, MapPin } from 'lucide-react';
import { hopeClinicLocations as SEED } from '@/data/hope-clinics';

interface Location { id: string; name: string; city: string; country: string; region: string; latitude: number; longitude: number; year_founded?: number; status: string; description?: string; phone?: string; address?: string; is_hq: boolean; order_index: number }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          <div><label className="block text-xs font-semibold text-[#0F2340] mb-1">Clinic Name *</label>
            <input value={form.name ?? ''} onChange={e => set('name', e.target.value)} className={inputCls} placeholder="e.g. Hope Clinic Koumé (HQ)" /></div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div><label className="block text-xs font-semibold text-[#0F2340] mb-1">Year Founded</label>
              <input type="number" value={form.year_founded ?? ''} onChange={e => set('year_founded', +e.target.value || undefined)} className={inputCls} placeholder="e.g. 2015" /></div>
            <div><label className="block text-xs font-semibold text-[#0F2340] mb-1">Phone</label>
              <input value={form.phone ?? ''} onChange={e => set('phone', e.target.value)} className={inputCls} placeholder="+237…" /></div>
          </div>
...
export default function NetworkManager() {
  ...
  useEffect(() => {
    fetch('/api/admin/network').then(r => r.ok ? r.json() : null).then(d => setLocs(d?.length ? d : toRows())).catch(() => setLocs(toRows())).finally(() => setLoading(false));
  }, []);

  const remove = (id: string) => { if (!confirm('Delete this location?')) return; setLocs(p => p.filter(l => l.id !== id)); fetch(`/api/admin/network/${id}`, { method: 'DELETE' }).catch(() => {}); };

  const save = async (form: Partial<Location>) => {
    try {
      const r = await fetch(form.id ? `/api/admin/network/${form.id}` : '/api/admin/network', { method: form.id ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
      const d = await r.json();
      setLocs(p => form.id ? p.map(x => x.id === d.id ? d : x) : [...p, d]);
    } catch { setLocs(p => form.id ? p.map(x => x.id === form.id ? { ...x, ...form } as Location : x) : [...p, { ...form, id: Date.now().toString() } as Location]); }
    setModal(null);
  };

  const displayed = filter === 'ALL' ? locs : locs.filter(l => l.status === filter || l.region === filter);
  const counts    = { ACTIVE: locs.filter(l => l.status === 'ACTIVE').length, PLANNED: locs.filter(l => l.status === 'PLANNED').length, IN_CONSTRUCTION: locs.filter(l => l.status === 'IN_CONSTRUCTION').length };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#8896B3] text-sm">{counts.ACTIVE} active · {counts.PLANNED} planned · {counts.IN_CONSTRUCTION} in construction</p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {[['ALL', `All (${locs.length})`], ['ACTIVE', `Active (${counts.ACTIVE})`], ['PLANNED', `Planned (${counts.PLANNED})`], ['IN_CONSTRUCTION', 'In Construction'], ...REGIONS.map(r => [r, r])].map(([val, label]) => (
                  ...
                  <tr key={loc.id} className="hover:bg-[#FAFBFF] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-[#0F2340] text-sm">{loc.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-[#4A5568] text-xs">{loc.city}, {loc.country}</td>
                    <td className="px-4 py-3 text-[#8896B3] text-xs">{loc.region}</td>
                    <td className="px-4 py-3 text-[#8896B3] text-xs">{loc.year_founded ?? '—'}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setModal(loc)} className="p-1.5 rounded-lg text-[#0F2340] hover:bg-[#EBF0FB] transition-colors"><Edit3 className="w-3.5 h-3.5" /></button>
                        <button onClick={() => remove(loc.id)} className="p-1.5 rounded-lg text-[#C8102E] hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
