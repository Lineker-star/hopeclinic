...
function Modal({ member, activeTab, onClose, onSave }: {
  ...
  const uploadPhoto = async (file: File) => {
    setUploading(true);
    const fd = new FormData(); fd.append('file', file); fd.append('folder', 'staff');
    try {
...
export default function StaffManager() {
  ...
  const save = async (form: Partial<StaffMember>) => {
    try {
      const r = await fetch(form.id ? `/api/admin/staff/${form.id}` : '/api/admin/staff', {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#8896B3] text-sm">{all.length} total staff across 6 categories</p>
        </div>
