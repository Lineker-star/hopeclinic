  ...
  return (
    <header
      className={`fixed left-0 right-0 z-40 transition-all duration-300 ${
        scrolled ? 'bg-[#0F2340] shadow-lg' : 'bg-[#0F2340]/97 backdrop-blur-sm'
      }`}
      style={{ top: '36px' }}
    >
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="hidden sm:block">
              <div className="font-bold text-white text-sm leading-tight tracking-wide"
                style={{ fontFamily: 'Cormorant Garamond, serif' }}>HOPE CLINIC</div>
              <div className="text-[#D4A017] text-[10px] leading-tight font-semibold tracking-widest">KOUMÉ — CMFI</div>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-2">
                  ...
                  {languages.map((lang) => (
                    <button key={lang.code}
                      ...
                      className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 hover:bg-[#EBF0FB] transition-colors ${
                        currentLang.code === lang.code ? 'text-[#0F2340] font-bold' : 'text-[#2D2D2D]'
                      }`}>
            ...
            {/* Book CTA */}
            <Link href="/booking"
              className="hidden md:flex items-center gap-1 bg-[#D4A017] text-[#0F2340] px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#F5C842] transition-colors shadow-md">
              Book Appointment
            </Link>
        ...
        {/* Mobile drawer */}
        {mobileOpen && (
          <div className="lg:hidden border-t border-white/10 py-3 bg-[#0F2340]">
            {navLinks.map(({ href, label }) => (
            ...
            <div className="px-4 pt-3 pb-1 border-t border-white/10 mt-2">
              <Link href="/booking" onClick={() => setMobileOpen(false)}
                className="w-full block text-center bg-[#D4A017] text-[#0F2340] px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-[#F5C842] transition-colors">
                Book Appointment
              </Link>
            </div>
          </div>
